...
class GForms {
    ...
    public function addField($form = array()) {
        ...
        $choices = array(
            array(
                'text' => self::getCheckboxText($form['id']) . ' <abbr class="psdsgvo-required" title="' . self::getRequiredMessage($form['id']) . '">*</abbr>',
                'value' => 'true',
                'isSelected' => false
            )
        );
        foreach ($form['fields'] as &$field) {
            ...
            if (isset($field->psdsgvo) && $field->psdsgvo === true) {
        ...
        if (!$isUpdated) {
            ...
            $args = array(
                'id' => $id,
                'type' => 'checkbox',
                'label' => __('Privacy', PS_DSGVO_C_SLUG),
            ...
            $form['fields'][] = apply_filters('psdsgvo_gforms_field_args', $args, $form);
        }
    ...
    public function displayAcceptedDateColumnInEntryOverview($columns = array(), $formId = 0) {
        ...
        if (!empty($key) && isset($columns[$key])) {
            $columns[$key] = apply_filters('psdsgvo_gforms_accepted_date_column_in_entry_overview', __('Privacy', PS_DSGVO_C_SLUG), $columns[$key], $formId);
        }
        return $columns;
    }
    ...
    public function displayAcceptedDateInEntryOverview($value = '', $formId = 0, $fieldId = 0, $entry = array()) {
        if (empty($value)) {
            ...
            if (!empty($id) && $fieldId === $id) {
                $value = (!empty($entry[$fieldId])) ? $entry[$fieldId] : __('Not accepted.', PS_DSGVO_C_SLUG);
                $value = apply_filters('psdsgvo_gforms_accepted_date_in_entry_overview', $value, $fieldId, $formId, $entry);
            }
        }
        return $value;
    }
    ...
    public function displayAcceptedDateInEntry($value, $entry = array()) {
        ...
        if (!empty($fieldId) && isset($value[$fieldId])) {
            if (empty($value[$fieldId])) {
                $value = __('Not accepted.', PS_DSGVO_C_SLUG);
            }
            $value = apply_filters('psdsgvo_gforms_accepted_date_in_entry', $value, $fieldId, $entry);
        }
        return $value;
    }
        ...
        if ($field instanceof \GF_Field) {
            if (isset($field['psdsgvo']) && $field['psdsgvo'] === true) {
                if (!empty($value)) {
                    ...
                    $value = sprintf(__('Accepted on %s.', PS_DSGVO_C_SLUG), $date);
                } else {
                    $value = __('Not accepted.', PS_DSGVO_C_SLUG);
                }
                $value = apply_filters('psdsgvo_gforms_accepted_date_to_entry', $value, $field, $lead);
            }
        }
        return $value;
    }
        ...
        foreach ($form['fields'] as &$field) {
            if (isset($field['psdsgvo']) && $field['psdsgvo'] === true) {
                if (isset($field['failed_validation']) && $field['failed_validation'] === true) {
                    $field['validation_message'] = apply_filters('psdsgvo_gforms_validation_message', self::getErrorMessage($form['id']), $field, $form);
                }
            }
        }
    ...
    public function getEnabledForms() {
        return (array)get_option(PS_DSGVO_C_PREFIX . '_integrations_' . self::ID . '_forms', array());
    }
    ...
    public function getFormTexts() {
        return (array)get_option(PS_DSGVO_C_PREFIX . '_integrations_' . self::ID . '_form_text', array());
    }
    ...
    public function getFormErrorMessages() {
        return (array)get_option(PS_DSGVO_C_PREFIX . '_integrations_' . self::ID . '_error_message', array());
    }
    ...
    public function getFormRequiredMessages() {
        return (array)get_option(PS_DSGVO_C_PREFIX . '_integrations_' . self::ID . '_required_message', array());
    }
    ...
    public function getCheckboxText($formId = 0, $insertPrivacyPolicyLink = true) {
        if (!empty($formId)) {
            ...
            if (!empty($texts[$formId])) {
                ...
                return apply_filters('psdsgvo_gforms_checkbox_text', $result, $formId);
            }
        }
    ...
    public function getErrorMessage($formId = 0) {
        if (!empty($formId)) {
            ...
            if (!empty($errors[$formId])) {
                ...
                return apply_filters('psdsgvo_gforms_error_message', $result, $formId);
            }
        }
    ...
    public function getRequiredMessage($formId = 0) {
        if (!empty($formId)) {
            ...
            if (!empty($errors[$formId])) {
                ...
                return apply_filters('psdsgvo_gforms_required_message', $result, $formId);
            }
        }
