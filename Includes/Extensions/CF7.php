...
class CF7 {
    ...
    public function addAcceptedDateToForms() {
        foreach ($this->getEnabledForms() as $formId) {
            ...
            if (!empty($output)) {
                $tag = '[psdsgvo]';
                $body = $output['body'];
                preg_match('/(\[psdsgvo\])/', $body, $matches);
                if (empty($matches)) {
                    $pattern = '/(--)/';
    ...
    /**
     * Remove [psdsgvo] string from disabled forms
     */
    ...
    public function addFormTagSupport() {
        wpcf7_add_form_tag('psdsgvo', array($this, 'addFormTagHandler'));
    }
    ...
    public function changeMailBodyOutput(\WPCF7_ContactForm $contactForm) {
        ...
        if (!empty($mail['body'])) {
            ...
            if (!empty($submission)) {
                ...
                if (isset($data['psdsgvo']) && $data['psdsgvo'] == 1) {
                    $value = Helper::localDateFormat(get_option('date_format') . ' ' . get_option('time_format'), time());
                } else {
                    $value = __('Not accepted.', PS_DSGVO_C_SLUG);
                }
                $output = apply_filters(
                    'psdsgvo_cf7_mail_body_output',
                    __('GDPR accepted on:', PS_DSGVO_C_SLUG) . "\n$value",
                    $data,
                    $submission
                );
                $mail['body'] = str_replace('[psdsgvo]', $output, $mail['body']);
    ...
    public function getCheckboxText($formId = 0, $insertPrivacyPolicyLink = true) {
        if (!empty($formId)) {
            ...
            if (!empty($texts[$formId])) {
                ...
                return apply_filters('psdsgvo_cf7_checkbox_text', $result, $formId);
            }
        }
    ...
    public function getErrorMessage($formId = 0) {
        if (!empty($formId)) {
            ...
            if (!empty($errors[$formId])) {
                ...
                return apply_filters('psdsgvo_cf7_error_message', $result, $formId);
            }
        }
